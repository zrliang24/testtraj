...
Now, use the *rasterize()* function from the terra package to create a frequency plot of the trajectory points. This function will count the number of times that each trajectory passes over each grid cell.  ![](/home/donatello/Rprojects/testtraj/vignettes/frequency_plot.png)
...
For the next step, you will need a basemap and a dataset containing measurements with associated dates, times, coordinates, and regions. The basemap used below is downloaded using the *ggmap* package. You will need an API key to download a *ggmap* basemap. Learn more about the *ggmap* basemaps [here](https://rdrr.io/cran/ggmap/man/get_map.html)
